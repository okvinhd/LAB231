...
<link href="https://fonts.googleapis.com/css2?family=Fredoka+One&display=swap" rel="stylesheet">
    ...
    <body >
        ...
        <div class="container">
            <header>
                <ul>
                    ...
                    <li><a href="#contact">Thực đơn</a></li>
                    <li><a href="#about">Combo món ăn</a></li>
                    <li><a id="alwaysHover" href="#news">Dịch vụ</a></li> 
            ...
            <form class="search" action="Service" method="GET">
                <div class="one-box">
                    <div class="box-search">Tên dịch vụ:</div>
                    <input name="nameService" class="txt-box" type="text" placeholder="Nhập tên dịch vụ" maxlength="50" value="${nameService}"/>
                </div>
                <div class="one-box">
                ...
                <a onclick="openForm()" class="btn-add"><i class="far fa-plus-square"></i>Thêm dịch vụ</a>
            </form>
                ...
                <div class="space-list">
                    <table class="list">
                        ...
                        <tr style="background-color:  #198754; height: 30px">
                            ...
                            <td class="title-list">Tên dịch vụ</td>
                            <td class="title-list" style="text-align: right;padding-right: 10%;">Giá</td>
                            <td class="title-list ">Loại dịch vụ</td>
            ...
            <c:forEach items="${listService}" var="ls">
                <form action="EditController?nameService=${nameService}&idTypeService=${idService}&index=${index}" method="POST" class="form-popup" id="detailservice${ls.id}">
                    <div class="header-popup">
                        <div class="title-popup">Chi tiết dịch vụ</div>
                    ...
                    <div class="main-popup">
                        ...
                        <div class="title-popup">Mã dịch vụ:</div>
                        <input class="box-popup" name="edit-idService" type="text" placeholder="Nhập tên dịch vụ" value="${ls.id}" readonly="readonly"/>
                        <div class="title-popup">Tên dịch vụ:</div>
                        <input oninput="checkEachInputEdit(this)" id="name-service-${ls.id}" class="box-popup" name="edit-nameService" type="text" maxlength="50" placeholder="Nhập tên dịch vụ" value="${ls.name}"/>
                        ...
                        <select class="combobox-popup" name="edit-statusService" class="combo-box">
                            <option value="true" ${ls.status eq "Đang hoạt động" ? 'selected' : ""}>Đang hoạt động</option>
                            <option value="false" ${ls.status ne "Đang hoạt động" ? 'selected' : ""}>Không hoạt động</option>
                        </select>
                        <div class="title-popup">Giá dịch vụ:</div>
                        <input oninput="checkEachInputEdit(this)" id="price-service-${ls.id}"  class="box-popup" name="edit-priceService" type="number" placeholder="Nhập giá dịch vụ" value="${ls.price}"/>
                        ...
                        <textarea oninput="changeDescriptionEdit(this)"  id="description-service-${ls.id}" class="box-popup" name="edit-descriptionService" maxlength="200" placeholder="Mô tả dịch vụ">${ls.decription}</textarea></div>
        ...
        <form action="AddController?nameService=${nameService}&idTypeService=${idService}&index=${index}" method="POST" class="form-popup" id="myForm">
            <div class="header-popup">
                <div class="title-popup">Thêm dịch vụ</div>
            ...
            <div class="main-popup">
                ...
                <div class="title-popup">Tên dịch vụ:</div>
                <input oninput="changeName()" id="name-service" class="box-popup" name="add-nameService" maxlength="50" type="text" placeholder="Nhập tên dịch vụ"/>
                ...
                <div class="title-popup">Giá dịch vụ:</div>
                <input id="price-service" oninput="changePrice(this)" class="box-popup" name="add-priceService" type="number" placeholder="Nhập giá dịch vụ" maxlength="8" minlength="4"/>
                <div id="warning-price" class="warning">Vui lòng nhập giá hợp lệ, nhỏ hơn 100.000.000 VNĐ và lớn hơn 999 VNĐ</div>
                <div class="title-popup">Mô tả:</div>
                <textarea maxlength="200" id="description-service" oninput="changeDescription()" class="box-popup" name="add-descriptionService" placeholder="Mô tả dịch vụ"></textarea>
        ...
        <c:if test="${update eq 'false'}">
            <script>
                alert("DỮ LIỆU VỪA NHẬP ĐÃ TRÙNG, VUI LÒNG thử");
            </script>
        </c:if>
        <c:choose>
